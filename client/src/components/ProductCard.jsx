...
const ProductCard = ({ product }) => {
  const { addToCart, removeFromCart, cartItem } = useContext(AppContext);
  const navigate = useNavigate();

  const handleNavigate = () => {
    navigate(
      `/products/${product.productCategory?.name?.toLowerCase()}/${product._id}`
    );
  };
  ...
  return product ? (
    ...
    >
      <div className="group cursor-pointer flex items-center  justify-center px-2">
        <img
          className="group-hover:scale-105 transition object-cover max-w-26 md:max-w-32"
